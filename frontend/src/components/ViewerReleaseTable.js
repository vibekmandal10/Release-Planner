...
const ViewerReleaseTable = ({
  ...
  }, [filterOptions.releaseVersions, isInitialLoad, onFilter]);

  // Apply filters locally to the releases data
  const filteredReleases = useMemo(() => {
    ...
    if (filters.release_version) {
      filtered = filtered.filter((r) => {
        const hasVersion =
          r.release_version &&
          r.release_version.includes(filters.release_version);
        if (!hasVersion && r.release_version) {
        }
        return hasVersion;
      });
    }
    ...
    if (filters.account_region) {
      const accountsInRegion = accounts
        .filter((acc) => acc.region === filters.account_region)
        .map((acc) => acc.name);
      filtered = filtered.filter((r) =>
  ...
  const sortedReleases = useMemo(() => {
    if (!sortConfig.key) return filteredReleases;
  ...
  }, [filteredReleases, sortConfig, accounts]);
  ...
  const handleFilterChange = (e) => {
    ...
    // Also call the parent's onFilter if it exists (for consistency)
    // if (onFilter) {
    //   onFilter(newFilters);
    // }
  };
...
export default ViewerReleaseTable;
